...
// Grundläggande felkontroll för kritiska element
if (!canvas || !ctx || !textCanvas || !textCtx || !input) {
    console.error("Ett eller flera nödvändiga HTML-element saknas!");
} else {
    // Anpassa storlek till fönstret
    ...
    let textCords = [];
    let particlesArray = [];
    const numberOfParticles = 4000; // Antal partiklar
    ...
    function drawText() {
        try {
            ...
            const text = input.value.trim() || " "; // Fallback om input är tom
            ...
            console.error("Fel vid ritning av text:", err);
        }
    }
        ...
        try {
            ...
            for (let y = 0; y < textCanvas.height; y += 4) {
                for (let x = 0; x < textCanvas.width; x += 4) {
                    ...
                    if (data[pixelIndex + 3] > 0) { // Alpha > 0 = finns text
                        cords.push({ x, y });
                    }
                }
            }
        } catch (err) {
            console.error("Fel vid avläsning av textpixlar:", err);
        }
        return cords;
    }
    ...
    class Particle {
        constructor() {
            ...
            // Om inga koordinater hittas, stanna på slumpad position
            const randomIndex = textCords.length > 0 ? Math.floor(Math.random() * textCords.length) : -1;
            this.target = randomIndex >= 0 ? textCords[randomIndex] : { x: this.x, y: this.y };
        }
        ...
        update() {
            ...
            // Lägg till grundhastighet (små variationer)
    ...
    function animate() {
        try {
            ...
            console.error("Fel i animation:", err);
        }
    }
